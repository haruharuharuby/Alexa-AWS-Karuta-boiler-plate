...
class ConventionallyDeployment
  ...
  def deploy_feature(feature)
    Dir.entries("../#{feature}")[2..-1].map{|action|
      deploy_function(feature, action) if File.ftype("../#{feature}/#{action}") == 'directory' && action != 'test'
    }
  end
  ...
  private
    ...
    def create_resource(name)
      ...
      policy.keys.map{|resource_name|
        self.send("create_resource_#{resource_name}")
        puts "[STATE::Resource] create resource using by lambda #{name} => #{resource_name} => #{policy[resource_name].join(",")}"
      }
    end


    def create_resource_dynamodb()
      dynamo = @aws.dynamo
      list = get_resource()[:dynamodb]
      list.map{|schema|
        dynamo.delete_table({table_name: schema[:table_name]}) if dynamo.list_tables()[:table_names].include?(schema[:table_name])
        sleep(10)
        dynamo.create_table(schema) unless dynamo.list_tables()[:table_names].include?(schema[:table_name])
        sleep(10)
      }
    end
    ...
    def link_resource_dynamodb(sender_lambda, receiver)
      ...
      for name in receiver
        ...
        if resp.table.global_secondary_indexes
          for index in resp.table.global_secondary_indexes
    ...
    def get_config(path)
      json_h = {}
      open(path){|f| json_h = JSON.parse(f.read, symbolize_names: true)}
      json_h
    end
